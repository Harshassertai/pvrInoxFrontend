...
function BarClickTable() {
	const [tableData, setTableData] = useState([]);
	const [tableTitle, setTableTitle] = useState("");
	const location = useLocation();
	const state = location.state;
	let { dates, zoneValue } = state;
    ...
    const [bulkStatusCommentsResponse, setBulkStatusCommentsResponse] =
      useState([]);
  ...
  const confirm = async (dataRecordOfStatus) => {
    ...
    if (comments.length == 0) {
      ...
      if (state.isApiCall == false) {
        ...
        let apiData = await BarClickListFor24hrs(state.zoneValue);
        setLoading(false);
  ...
  const handleBulkCloseCommentClick = async (recordsForClose) => {
  ...
  return (
    <>
      ...
      <Modal
        ...
        onOk={async () => {
          console.log("Rows Document", modalRecord);
          let { usersAssigned } = modalRecord;
          if (usersAssigned != name) {
            openNotification("You cannot close this status.[NOT ASSIGNED]");
            return;
          } else {
            ...
            if (state.isApiCall == false) {
              let updatedTableData = await BarClickListFor24hrs(
                state.zoneValue
              );
              let newData = updatedTableData.data
                .filter((item) => {
                  if (item.status == "Open") {
                    return item;
                  }
              ...
              let updatedTableData = await BarGraphClick({
                startingDateValue,
                startingTimeValue,
                endingDateValue,
                endingTimeValue,
        ...
        onCancel={async () => {
          ...
          let zoneSelected = state.zoneValue;
          let usersSelected = rowsValues.map((item) => {
            return item.usersAssigned;
          });
          ...
          <Form.Item>
            <div style={{ display: "flex", alignItems: "center", gap: "2vh" }}>
              ...
              <Button
                type="primary"
                onClick={() => {
                  handleBulkCloseCommentClick(rowsForBulkClose);
                }}
                disabled={
                  modalRecord?.status == "Close" ||
                  modalRecord?.status == "Closed"
